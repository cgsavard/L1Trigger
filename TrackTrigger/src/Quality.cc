...
void Quality::Prediction(TTTrack < Ref_Phase2TrackerDigi_ > &aTrack) {
  if (this->Algorithm_ == "Cut"){
  ...
  else if (this->Algorithm_ == "None"){
    aTrack.settrkMVA1(-999);
  }

  else if (this->Algorithm_ == "MVA"){
    ...
    // vector of float to create a 1,1,n_features ortinput
    ...
    // grab last possible element in outputs (probability)
    aTrack.settrkMVA1(ortoutputs.back().back());
  }
